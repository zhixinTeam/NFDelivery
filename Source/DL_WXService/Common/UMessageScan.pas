...
type
  ...
  protected
    ...
    //销售出厂消息
    procedure DoSaveInFactMsg;
    //执行进厂消息插入
    function SaveSaleInFactMsg(nList: TStrings):Boolean;
    //销售进厂消息
      ...
      begin
        ...
        begin
          ...
          try
            ...
            nResult := TBusWorkerBusinessWebchatNC.CallMe(cBC_WX_complete_shoporders
                       ,nStr,'',@nOut);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        Result := TBusWorkerBusinessWebchatNC.CallMe(cBC_WX_send_event_msg
           ,nStr,'',@nOut);
        Next;
      end;
    end;
...
procedure TMessageScanThread.DoSaveInFactMsg;
var nStr: string;
    nInit: Int64;
    nErr,nIdx: Integer;
    nOut: TWorkerWebChatData;
begin
  ...
  begin
    ...
    FListA.Text := StringReplace(nStr, '\S', #$D#$A, [rfReplaceAll]);
    if FListA.Values['WOM_BillType'] = sFlag_Sale then
      SaveSaleInFactMsg(FListA);
  end;
  WriteLog('插入进厂推送消息耗时: ' + IntToStr(GetTickCount - nInit) + 'ms');
end;
...
begin
  ...
  nStr := 'select WOM_LID from %s where WOM_LID=''%s'' and WOM_StatusType=%d ';
  nStr := Format(nStr,[sTable_WebOrderMatch,nLID,c_WeChatStatusIn]);
  //xxxxx
