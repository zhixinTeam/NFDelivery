...
type
  TfFrameCardTmpPQuery = class(TfFrameNormal)
...
class function TfFrameCardTmpPQuery.FrameID: integer;
begin
  Result := cFI_FrameCardTmpPQuery;
end;

procedure TfFrameCardTmpPQuery.OnCreateFrame;
begin
  inherited;
...
procedure TfFrameCardTmpPQuery.OnDestroyFrame;
begin
  SaveDateRange(Name, FStart, FEnd);
  inherited;
end;

function TfFrameCardTmpPQuery.InitFormDataSQL(const nWhere: string): string;
begin
  ...
  Result := MacroValue(Result, [MI('$TYPE', sFlag_ShipTmp),
...
procedure TfFrameCardTmpPQuery.AfterInitFormData;
begin
  FJBWhere := '';
end;

//Desc: 日期筛选
procedure TfFrameCardTmpPQuery.EditDatePropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
//Desc: 执行查询
procedure TfFrameCardTmpPQuery.EditTruckPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
procedure TfFrameCardTmpPQuery.Check1Click(Sender: TObject);
begin
  BtnRefresh.Click;
end;
...
//------------------------------------------------------------------------------
//Desc: 权限控制
procedure TfFrameCardTmpPQuery.PMenu1Popup(Sender: TObject);
begin
  N3.Enabled := BtnPrint.Enabled and (not Check1.Checked);
end;

//Desc: 打印磅单
procedure TfFrameCardTmpPQuery.N3Click(Sender: TObject);
var nStr: string;
begin
...
//Desc: 时间段查询
procedure TfFrameCardTmpPQuery.N2Click(Sender: TObject);
begin
...
//Desc: 销售完成统计
procedure TfFrameCardTmpPQuery.N6Click(Sender: TObject);
begin
  if ShowDateFilterForm(FTimeS, FTimeE, True) then
  try
...
//Desc: 采购完成统计
procedure TfFrameCardTmpPQuery.N7Click(Sender: TObject);
begin
  if ShowDateFilterForm(FTimeS, FTimeE, True) then
  try
...
//Desc: 删除榜单
procedure TfFrameCardTmpPQuery.BtnDelClick(Sender: TObject);
var nIdx: Integer;
    nStr,nID,nP: string;
begin
...
//Desc: 查看抓拍
procedure TfFrameCardTmpPQuery.N4Click(Sender: TObject);
...
initialization
  gControlManager.RegCtrl(TfFrameCardTmpPQuery, TfFrameCardTmpPQuery.FrameID);
end.
