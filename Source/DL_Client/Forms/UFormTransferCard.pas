...
type
  TfFormTransferCard = class(TfFormNormal)
...
class function TfFormTransferCard.FormID: integer;
begin
  Result := cFI_FormMakeTranCard;
end;

class function TfFormTransferCard.CreateForm(const nPopedom: string;
  const nParam: Pointer): TWinControl;
begin
  Result := nil;
  if not Assigned(nParam) then Exit;

  with TfFormTransferCard.Create(Application) do
  try
    FParam := nParam;
    InitFormData;
    ActionComPort(False);
...
procedure TfFormTransferCard.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  ActionComPort(True);
end;

procedure TfFormTransferCard.InitFormData;
begin
...
//Desc: 串口操作
procedure TfFormTransferCard.ActionComPort(const nStop: Boolean);
var nInt: Integer;
    nIni: TIniFile;
begin
  if nStop then
...
procedure TfFormTransferCard.ComPort1RxChar(Sender: TObject; Count: Integer);
var nStr: string;
    nIdx,nLen: Integer;
begin
...
procedure TfFormTransferCard.EditCardKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
    Key := #0;
...
//Desc: 保存磁卡
procedure TfFormTransferCard.BtnOKClick(Sender: TObject);
var nRet: Boolean;
begin
  ...
  nRet := SaveDuanDaoCard(Trim(EditTruck.Text), EditCard.Text);
  if nRet then
    ModalResult := mrOk;
  //done
end;
...
initialization
  gControlManager.RegCtrl(TfFormTransferCard, TfFormTransferCard.FormID);
end.
