...
type
  TfFramePoundQueryView = class(TfFrameNormal)
...
class function TfFramePoundQueryView.FrameID: integer;
begin
  Result := cFI_FramePoundQueryView;
end;

procedure TfFramePoundQueryView.OnCreateFrame;
begin
  inherited;
...
procedure TfFramePoundQueryView.OnDestroyFrame;
begin
  SaveDateRange(Name, FStart, FEnd);
  inherited;
end;

function TfFramePoundQueryView.InitFormDataSQL(const nWhere: string): string;
var nStr: string;
begin
...
procedure TfFramePoundQueryView.AfterInitFormData;
begin
  FJBWhere := '';
end;

//Desc: 日期筛选
procedure TfFramePoundQueryView.EditDatePropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
//Desc: 执行查询
procedure TfFramePoundQueryView.EditTruckPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
procedure TfFramePoundQueryView.Check1Click(Sender: TObject);
begin
  BtnRefresh.Click;
end;
...
//------------------------------------------------------------------------------
//Desc: 权限控制
procedure TfFramePoundQueryView.PMenu1Popup(Sender: TObject);
begin
...
//Desc: 打印磅单
procedure TfFramePoundQueryView.N3Click(Sender: TObject);
var nStr: string;
begin
...
//Desc: 时间段查询
procedure TfFramePoundQueryView.N2Click(Sender: TObject);
begin
...
//Desc: 销售完成统计
procedure TfFramePoundQueryView.N6Click(Sender: TObject);
begin
  if ShowDateFilterForm(FTimeS, FTimeE, True) then
  try
...
//Desc: 采购完成统计
procedure TfFramePoundQueryView.N7Click(Sender: TObject);
begin
  if ShowDateFilterForm(FTimeS, FTimeE, True) then
  try
...
//Desc: 删除榜单
procedure TfFramePoundQueryView.BtnDelClick(Sender: TObject);
var nIdx: Integer;
    nStr,nID,nP: string;
begin
...
//Desc: 查看抓拍
procedure TfFramePoundQueryView.N4Click(Sender: TObject);
...
procedure TfFramePoundQueryView.N8Click(Sender: TObject);
begin
  inherited;
  if ShowDateFilterForm(FTimeS, FTimeE, True) then
  try
...
procedure TfFramePoundQueryView.N9Click(Sender: TObject);
var nStr,nP: string;
begin
  if cxView1.DataController.GetSelectedCount < 1 then
  begin
...
//Desc: 磅单勘误
procedure TfFramePoundQueryView.N11Click(Sender: TObject);
...
//Desc: 船运采购单
procedure TfFramePoundQueryView.N12Click(Sender: TObject);
var nP: TFormCommandParam;
begin
  if cxView1.DataController.GetSelectedCount < 1 then
  begin
...
initialization
  gControlManager.RegCtrl(TfFramePoundQueryView, TfFramePoundQueryView.FrameID);
end.
