...
begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if nStr = sFlag_ESBSrv then
          gSysParam.FESBSrv := Fields[0].AsString;
        //local mit

        if nStr = sFlag_ESBDB then
          gSysParam.FESBDB := Fields[0].AsString;
        //local mit

        if nStr = sFlag_ESBLog then
          gSysParam.FESBLog := Fields[0].AsString;
        //local mit

        if nStr = sFlag_ESBPass then
          gSysParam.FESBPass := Fields[0].AsString;
        //local mit
        Next;
      end;
    end;
  ...
  begin
    nStr := '未配置[ %s.%s ]字典项.';
    nStr := Format(nStr, [sTable_SysDict, sFlag_WXFactory]);
